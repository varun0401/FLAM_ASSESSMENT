    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 p-4">
      {employees.map(emp => (
        <div key={emp.id} className="bg-white dark:bg-gray-800 p-4 rounded shadow">
          <h2 className="text-xl font-semibold">{emp.firstName} {emp.lastName}</h2>
          <p className="text-sm text-gray-500">{emp.email}</p>
          <p className="text-sm">Department: {emp.department}</p>
          <p className="text-sm">Age: {emp.age}</p>
          <p className="text-yellow-400">⭐ {emp.rating} / 5</p>
          <div className="flex gap-2 mt-2">
            ...
            <button
              ...
              className={`${bookmarks.includes(emp.id) ? 'bg-red-500' : 'bg-green-500'
